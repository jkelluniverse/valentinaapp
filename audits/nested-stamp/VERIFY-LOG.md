...
Run: 2026-09-05T22:09:52.042Z · `npx tsx audits/nested-stamp-verify.ts`
...
# C24-NESTED-STAMP verify — 2026-09-05T22:09:49.751Z
- ✓ simulated request scope is real (next/headers resolves inside it)

## Verify 1 — the five assumptions
- ✓ the nested-write scanner is not blind — it finds the ones THIS harness writes deliberately — 16 in audits/nested-stamp-verify.ts
- ✓ A1 CORRECTED: outside this harness the repo contains ZERO nested relation writes on scoped models — 462 files scanned against 43 schema relation fields · hits=0
...
- ✓ A2 CORRECTED: it is NOT the only place a tenant is stamped — call sites stamp explicitly for the OUT-of-request path — 10 file(s), e.g. lib/engage.ts, audits/capture/verify.ts, audits/engage/verify.ts
...
- ✓ it records exactly the rows it stamped, per table (this is what makes it reversible) — Chapter=1 LogEntry=10
