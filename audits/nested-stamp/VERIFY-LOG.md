...
Run: 2026-09-23T18:50:27.783Z · `npx tsx audits/nested-stamp-verify.ts`
...
# C24-NESTED-STAMP verify — 2026-09-23T18:50:24.103Z
...
- ✓ A1 CORRECTED: outside the acceptance harnesses the repo contains ZERO nested relation writes on scoped models — 507 files scanned against 44 schema relation fields · hits=0
...
- ✓ A2 CORRECTED: it is NOT the only place a tenant is stamped — call sites stamp explicitly for the OUT-of-request path — 16 file(s), e.g. audits/c12x-ai-pass/run3-patch01.ts, audits/capture/verify.ts, audits/engage/verify.ts
...
- ✓ A3 note: the only tenantId columns OUTSIDE the scoped set are platform-level, deliberately — TenantDomain, TenantModule, PractitionerProspect
...
- ✓ it records exactly the rows it stamped, per table (this is what makes it reversible) — ActivityEvent=1 Chapter=1 LogEntry=84
