...
Run: 2026-09-15T21:16:50.405Z · `npx tsx audits/practice-setting-verify.ts`
...
# C25-PRACTICE-SETTING-TENANCY verify — 2026-09-15T21:16:30.187Z
...
- ✓ two ACTIVE non-default practices exist, provisioned by lib/signup.ts exactly as a founding practitioner creates one — cmu369cbw0001hberp7u5dwq0 (ACTIVE) · cmu369cmh0009hber2g200ryh (ACTIVE)

## Verify 2 (after) + 4 — the write succeeds, and two practices hold the same key independently
- ✓ AFTER — the exact write this spec exists to fix now SUCCEEDS for a non-default tenant, and is stamped to that tenant — 3 rows for the one key: cmu369cbw0001hberp7u5dwq0=A-value · cmu369cmh0009hber2g200ryh=B-value · tnt_valentina_000000001=default-value
...
- ✓ V8 — on a normal `id`-keyed model the pre-check still refuses a cross-tenant unique write, and the target row is untouched — tenant-scope: patternArchetype.upsert target belongs to another tenant · target still b/cmu369cmh0009hber2g200ryh
...
- ✓ V3 — the setting is WRITTEN inside a real request on her own host, owned by HER practice — rows before=0 · after: value=off tenantId=cmu369cbw0001hberp7u5dwq0 · url /practitioner/settings?saved=assist
...
- ✓ V3/V4 — both practices now hold `assistNotifyEmail` independently, each written through its own real request — cmu369cbw0001hberp7u5dwq0=off · cmu369cmh0009hber2g200ryh=off
