...
async function main() {
  ...
  // P3.3 — this gate drives app code OUT of a request, where there is no host
  // and therefore no tenant. It states its own, with the same withTenantScope
  // the data layer has required since C24.1 (ruling 24) and which the chrome
  // resolver now honours too. Without it loadFlowSchema() resolves the
  // unresolved shell and the drift checks compare the wrong tenant's schema.
  await withTenantScope(TENANT, async () => {
    try {
