...
const ALLOW_RAW_IMPORT: Record<string, string> = {
  ...
  "app/founders/apply/actions.ts": "ONE write: applicationMeta on the PractitionerProspect the shared captureProspect() just upserted. PractitionerProspect is a PLATFORM-LEVEL ledger and is deliberately absent from SCOPED_MODELS — its tenantId column is the tenant a prospect OWNS after conversion, not a scope column — so the scoped client cannot write it. The row is addressed by its unique email, never by a host-resolved tenant.",
  "audits/founders-verify.ts": "C35 acceptance harness: CLI-only. Reads back the prospect row it created through the real form to prove the STORED source is ruling 181's tag rather than \"web\", and removes its own probe row first and last.",
  ...
  "audits/tenant-scope-verify.ts": "C24.1-TENANT-SCOPE acceptance harness: CLI-only, must read tenantId columns RAW (a scoped read would hide the very rows it exists to see), must write a deliberately foreign tenantId to prove the scope leaves it alone, and must prove the UNSCOPED path still writes a null row; self-cleaning",
  ...
  "audits/practice-setting-verify.ts": "C25-PRACTICE-SETTING-TENANCY acceptance harness: CLI + browser-driven, must read tenantId columns RAW (a scoped read would hide the very rows it exists to see), must create a deliberate null-tenant row to give the migration's backfill something to count, and must reverse + re-apply migration 49 against the live database; self-cleaning",
