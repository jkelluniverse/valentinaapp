...
- MATERIAL KINDS: items of kind SESSION_NOTE are the coach's own in-session observations (id-prefixed "note:") — evidence-eligible, they carry what she saw in the room. Items of kind SPOKEN (id-prefixed "t:") are the client's own words spoken in session — the STRONGEST evidence available; prefer citing them when they carry a theme. All other kinds are the client's written material. Cite ids exactly as given, whatever the prefix.
- SENSITIVE AREAS (C12X §9 — require DIRECT evidence, flag rather than infer, never speculate): trauma history · abuse · psychiatric or medical diagnoses · sexuality · religion · allegations about family members · medical conditions · legal matters · risk of harm · third parties who have not consented. A report about the client from another person is context at most — never evidence on its own. If material only hints at one of these areas, do not build a node on it.
- THE CENTRAL PRINCIPLE: the client is not the chart, the diagnosis, the belief, the pattern, or the map. Every node is a hypothesis in service of a human being who exceeds all of them.
